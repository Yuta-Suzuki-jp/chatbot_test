  ...
  honda: {
    ...
    scenarios: {
      step1: {
        ...
        options: [
          { text: '「そろそろ買い替えかな」と思い始めた人が、最初に確認すべきこととは？', next: 'step2' },
          { text: 'ガソリン代が家計を直撃。EVに乗り換えた人が実感した「年間いくら変わった？」', next: 'step2' },
          { text: '子供を乗せるなら見ておきたい。最新の「安全装備」で何が変わったのか？', next: 'step2' }
        ]
      },
      step2: {
  ...
  bank: {
    ...
    scenarios: {
      step1: {
        ...
        options: [
          { text: '確定申告を余裕で終わらせる人と、毎年パニックになる人の「決定的な違い」とは？', next: 'step2' },
          { text: 'フリーランスが取引先に「信頼される口座」と「なんとなく不安な口座」の違いとは？', next: 'step2' },
          { text: '事業用口座を持つだけで節税になる？個人事業主が知らないと損するお金の話', next: 'step2' }
        ]
      },
      step2: {
  ...
  card: {
    ...
    scenarios: {
      step1: {
        ...
        options: [
          { text: '急な出費が重なったとき、賢い人はどうやって乗り越えているのか？', next: 'step2' },
          { text: '給料日前でも「欲しいものを手に入れる」賢い人たちのお金の使い方とは？', next: 'step2' },
          { text: '毎月の返済、実は減らせる？カードローン「乗り換え」で変わること', next: 'step2' }
        ]
      },
      step2: {
  ...
  insurance: {
    ...
    scenarios: {
      step1: {
        ...
        options: [
          { text: '「なんとなく更新」していませんか？自動車保険を見直すべきタイミングとは？', next: 'step2' },
          { text: '同じ補償なのに保険料が違う理由。自動車保険の「選び方」で変わることとは？', next: 'step2' },
          { text: '事故が起きたとき「補償が足りなかった」と後悔する人の共通点とは？', next: 'step2' }
        ]
      },
      step2: {
        ...
        <div className="w-full px-3 py-2 overflow-x-auto scrollbar-hide">
          ...
          {Object.values(useCases).map((uc) => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold transition-all whitespace-nowrap ${activeCaseKey === uc.id
